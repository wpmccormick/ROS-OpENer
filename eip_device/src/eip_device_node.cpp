...
int main(int argc, char **argv)
{
  ...
  ros::Publisher eip_data_pub = nh.advertise<eip_device::EipDataAsmOut>("eip_data_fmplc", 1);

  //a topic to publish to for config FROM the PLC
  ros::Publisher eip_config_pub = nh.advertise<eip_device::EipDataAsmCfg>("eip_data_config", 1);

  //a topic to publish to for data FROM the PLC
  ros::Publisher eip_status_pub = nh.advertise<eip_device::EipDeviceStatus>("eip_status", 100);

  //a top to subcribe to for UNSOLICITED data TO the PLC
  ...
  while(ros::ok() && (1 != g_end_stack) && (EipStatus == kEipStatusOk))2
  {
    ...
    eipDevice.data_asm_out.raw_plc_data.assign(g_assembly_data096, g_assembly_data096 + OUTPUT_ASSEMBLY_BUFFER_SIZE);

    //copy data from the config assembly array to the message topic
    eipDevice.data_asm_cfg.raw_plc_data.assign(g_assembly_data097, g_assembly_data097 + CONFIG_ASSEMBLY_BUFFER_SIZE);

    //and publish
    eip_data_pub.publish(eipDevice.data_asm_out);
    eip_config_pub.publish(eipDevice.data_asm_cfg);
